...
│       └── include/
│           └── cicada.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cicada.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cicada.h
```
...
int main() {
    void* handle = cicada_init();
    if (!handle) return 1;

    int result = cicada_process(handle, 42);
    if (result != 0) {
        const char* err = cicada_last_error();
...
Compile with:
```bash
gcc -o example example.c -lcicada -L./zig-out/lib
```

### From Idris2

```idris
import CICADA.ABI.Foreign
...
#[link(name = "cicada")]
extern "C" {
    fn cicada_init() -> *mut std::ffi::c_void;
    fn cicada_free(handle: *mut std::ffi::c_void);
    fn cicada_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cicada_init();
        assert!(!handle.is_null());

        let result = cicada_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcicada = "libcicada"

function init()
    handle = ccall((:cicada_init, libcicada), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cicada_process, libcicada), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cicada_free, libcicada), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/cicada.h
   ```
