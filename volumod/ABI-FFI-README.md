...
│       └── include/
│           └── volumod.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── volumod.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/volumod.h
```
...
int main() {
    void* handle = volumod_init();
    if (!handle) return 1;

    int result = volumod_process(handle, 42);
    if (result != 0) {
        const char* err = volumod_last_error();
...
Compile with:
```bash
gcc -o example example.c -lvolumod -L./zig-out/lib
```

### From Idris2

```idris
import VOLUMOD.ABI.Foreign
...
#[link(name = "volumod")]
extern "C" {
    fn volumod_init() -> *mut std::ffi::c_void;
    fn volumod_free(handle: *mut std::ffi::c_void);
    fn volumod_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = volumod_init();
        assert!(!handle.is_null());

        let result = volumod_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libvolumod = "libvolumod"

function init()
    handle = ccall((:volumod_init, libvolumod), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:volumod_process, libvolumod), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:volumod_free, libvolumod), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/volumod.h
   ```
