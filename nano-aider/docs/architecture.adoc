...
nano-aider is designed as a modular Ada application using the child
package pattern for clean separation of concerns.
...
[arabic]
. *Ada 2022 Standard*: Modern language features for safety
. *SPARK Annotations*: Critical sections formally verified
. *Minimal Dependencies*: Only ncursesada required
. *No Heap Allocation*: Stack-based where possible
. *Cross-Platform*: Linux, macOS, BSD, Windows (MSYS2)
