...
# TOTAL_UPDATE ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libtotal-update.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── total-update.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── total-update.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/total-update.h
```
...
```c
#include "total-update.h"

int main() {
    void* handle = total-update_init();
    if (!handle) return 1;

    int result = total-update_process(handle, 42);
    if (result != 0) {
        const char* err = total-update_last_error();
...
Compile with:
```bash
gcc -o example example.c -ltotal-update -L./zig-out/lib
```

### From Idris2

```idris
import TOTAL_UPDATE.ABI.Foreign
...
```rust
#[link(name = "total-update")]
extern "C" {
    fn total-update_init() -> *mut std::ffi::c_void;
    fn total-update_free(handle: *mut std::ffi::c_void);
    fn total-update_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = total-update_init();
        assert!(!handle.is_null());

        let result = total-update_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libtotal-update = "libtotal-update"

function init()
    handle = ccall((:total-update_init, libtotal-update), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:total-update_process, libtotal-update), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:total-update_free, libtotal-update), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/total-update.h
   ```
