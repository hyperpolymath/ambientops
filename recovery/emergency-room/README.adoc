...
* Migration: zig sources in `src/*.v` are deprecated and retained for reference only.
  See the `SIDELINED.adoc` in `src/` and link:../emergency-button/src/MIGRATION.adoc[emergency-button/src/MIGRATION.adoc].

NOTE: V was the original implementation language. zig was banned estate-wide on 2026-04-10;
...
`../emergency-button/src/zig/`. This mirrors the existing zig symlink pattern in
