...
/// Delay between reconnection attempts in nanoseconds (2 seconds).
const RECONNECT_DELAY_NS: u64 = 2 * std.time.ns_per_s;
...
pub const Connection = struct {
    ...
    /// Whether the connection is currently alive. Set to false when a
    /// disconnect is detected, triggering reconnection logic in the
    /// main dispatch loop.
    connected: bool = true,

    /// Number of consecutive reconnection attempts since the last
    /// successful connection. Reset to 0 on successful reconnect.
    ...
    pub fn init() !Connection {
        ...
        return Connection{ .raw = raw, .connected = true, .reconnect_attempts = 0 };
    }
    ...
    /// Attempt to reconnect to the session bus after a disconnection.
    ///
    /// This tears down the old connection and establishes a new one.
    /// On success, re-requests the well-known bus name and re-registers
    /// the method filter. Returns false if the reconnection attempt
    /// failed (caller should wait and retry).
    ///
    /// The reconnection attempt counter is tracked so the caller can
    /// implement a backoff strategy or give up after too many failures.
    pub fn reconnect(self: *Connection) bool {
        self.reconnect_attempts += 1;

        if (self.reconnect_attempts > MAX_RECONNECT_ATTEMPTS) {
            std.log.err(
                "DBus reconnection failed after {d} attempts — giving up",
        ...
        std.log.warn(
            "DBus reconnection attempt {d}/{d}...",
            .{ self.reconnect_attempts, MAX_RECONNECT_ATTEMPTS },
        );

        // Release the old (broken) connection handle.
        dbus_connection_unref(self.raw);

        // Attempt to get a fresh session bus connection.
        var err: DBusError = .{};
        dbus_error_init(&err);
        defer dbus_error_free(&err);
        ...
        const raw = dbus_bus_get(DBUS_BUS_SESSION, &err) orelse {
            if (dbus_error_is_set(&err) != 0) {
                if (err.message) |msg| {
                    std.log.warn(
                        "DBus reconnect: bus_get failed: {s}",
                        .{std.mem.span(msg)},
                    );
                }
            }
            // Mark as disconnected — caller will retry.
            self.connected = false;
            return false;
        };
        ...
        // Re-request our well-known name.
        self.requestName(BUS_NAME) catch |req_err| {
            std.log.warn(
                "DBus reconnect: name request failed: {any}",
                .{req_err},
            );
            // Connection is alive, but we don't own the name. The tray
            // can still receive signals; name ownership may recover on
            // a subsequent attempt.
        };

        // Re-register the method filter for incoming calls.
        registerMethodHandlers(self) catch |filter_err| {
            std.log.warn(
                "DBus reconnect: filter registration failed: {any}",
                .{filter_err},
            );
        };

        std.log.info("DBus reconnection successful", .{});
        return true;
    }
    ...
    /// Process pending DBus messages with a bounded timeout.
    ///
    ...
    /// for data. To prevent the indefinite-poll hang that causes
    /// SIGABRT via the systemd watchdog, the timeout is clamped to
    /// a maximum of 5000ms. A value of -1 (block forever) is
    /// explicitly rejected.
    ///
    /// When the connection is marked as disconnected, this returns
    /// error.DBusDisconnected immediately without touching libdbus.
    pub fn processMessages(self: *Connection, timeout_ms: i32) !void {
        // Guard: if we already know the connection is dead, don't call
        // into libdbus (which may hang on a broken socket).
        if (!self.connected) {
            return error.DBusDisconnected;
        }

        // Clamp timeout to prevent indefinite blocking. The maximum of
        // 5000ms ensures we return to the caller frequently enough to
        // send watchdog heartbeats and check shutdown flags.
        const max_timeout_ms: i32 = 5000;
        const safe_timeout: i32 = if (timeout_ms < 0)
            max_timeout_ms
        else
            @min(timeout_ms, max_timeout_ms);

        const result = dbus_connection_read_write_dispatch(
            self.raw,
            @intCast(safe_timeout),
        );

        if (result == 0) {
            // Connection has been disconnected by the bus daemon.
            self.connected = false;
            std.log.warn("DBus connection lost (read_write_dispatch returned 0)", .{});
            return error.DBusDisconnected;
        }
    }
