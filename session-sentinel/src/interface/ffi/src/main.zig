...
pub fn main() !void {
    ...
    if (config.daemon or !config.once) {
        ...
        // Main thread: process DBus messages with watchdog heartbeats.
        //
        // This is the critical loop that previously hung indefinitely in
        // __poll when the session bus became unavailable during
        // suspend/resume. The fix ensures:
        //   1. processMessages uses a bounded timeout (max 5s, here 100ms)
        //   2. Watchdog heartbeats are sent from the main thread, not just
        //      the monitor thread — so even if the monitor is sleeping,
        //      the watchdog stays alive.
        //   3. On DBusDisconnected, we attempt reconnection with backoff
        //      instead of spinning on a dead connection.
        //   4. The heartbeat file is written from the main thread as a
        //      secondary liveness signal.
        var watchdog_tick_ns: i128 = std.time.nanoTimestamp();
        const watchdog_heartbeat_interval_ns: i128 = 30 * std.time.ns_per_s;

        while (!shutdown_requested.load(.acquire)) {
            bus.processMessages(100) catch {
                // The only error processMessages returns is DBusDisconnected.
                // When the session bus drops (suspend/resume, bus restart),
                // we attempt reconnection with exponential backoff.
                std.log.warn("DBus disconnected — attempting reconnection", .{});

                // Send watchdog heartbeat before blocking on reconnect
                // so systemd knows we are still alive and recovering.
                wd.notifyWatchdog();

                // Back off with increasing delay between attempts.
                // Starts at 2s, caps at 30s per attempt.
                const delay_ns: u64 = @min(
                    @as(u64, bus.reconnect_attempts + 1) * (2 * std.time.ns_per_s),
                    30 * std.time.ns_per_s,
                );
                std.Thread.sleep(delay_ns);

                if (!bus.reconnect()) {
                    // All reconnection attempts exhausted. Shut down
                    // cleanly and let systemd restart us.
                    std.log.err("DBus reconnection exhausted — requesting shutdown", .{});
                    shutdown_requested.store(true, .release);
                }
            };
            ...
            // Periodically send watchdog heartbeat from the main thread.
            // This prevents the watchdog timeout (WatchdogSec=600) from
            // killing us with SIGABRT when the monitor thread is sleeping
            // between scan intervals.
            const now_ns = std.time.nanoTimestamp();
            if (now_ns - watchdog_tick_ns >= watchdog_heartbeat_interval_ns) {
                wd.notifyWatchdog();
                wd.writeHeartbeat() catch {};
                watchdog_tick_ns = now_ns;
            }
        }
