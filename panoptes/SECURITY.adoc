...
. Send a detailed report to: *security@panoptes.example.com* (or open a
...
[arabic]
. *Memory Safety*: Written in Rust with no `+unsafe+` blocks
. *Minimal Privileges*: Runs as non-root user in containers
...
[arabic]
. *File System Permissions*: Panoptes operates with user permissions;
...
. *Model Trust*: We use Moondream from official Ollama registry; verify
model integrity
