...
== AmbientOps Primary (AffineScript + Deno)

The AmbientOps umbrella and Ward UI are AffineScript-first.
...
Everything else stays AffineScript-first (or D/V as appropriate for the layer).
...
If wire participants are AffineScript/JS + Rust:

* Schemas: `ambientops-protocol/schemas/*.bop`
* Generated JS: `gen/js/` (consumed by AffineScript wrappers)
