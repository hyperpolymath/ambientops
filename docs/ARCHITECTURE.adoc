...
| Application UI | AffineScript (compiles to JS)
...
| nafa-app | Ward UI | AffineScript/Deno mobile journey planner, consumes system-weather
| panic-attacker | Diagnostics | Software health scanner → verisim
| verisim / verisim-data | Data Pipeline | Multimodal scan result database
