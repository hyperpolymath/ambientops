...
Network Orchestrator is a comprehensive tool for managing enterprise network infrastructure—routers, switches, DHCP, BGP, distributed filesystems, and remote diagnostics. Unlike vendor-specific management platforms or error-prone manual CLI work, it provides *safe, auditable, multi-vendor* network operations with *full rollback capabilities*.

[quote]
____
"It's 3am, your data center BGP session is flapping, DHCP is misconfigured, and your on-call engineer is unreachable. You don't need a CCIE—you need Network Orchestrator."
____
...
* **Vendor lock-in**: Cisco Prime, Juniper Space, Arista CVP—each requires separate training and licensing
...
Network Orchestrator provides *unified, safe, auditable management* of enterprise network infrastructure—across all major vendors and protocols.

=== 🌐 BGP Control & Monitoring
...
|Cisco IOS/IOS-XE, JunOS, Arista EOS, VyOS, FRRouting—unified syntax
|===
...
=== 🔧 Multi-Vendor Router/Switch Configuration
...
=== 💾 Distributed Filesystem Diagnostics
...
[✓] Syntax valid
[✓] Config diff: 3 lines added, 0 removed
...
├── core-rt1/
│   ├── 2026-01-29_14-30-22.cfg    # Backup before BGP change
│   ├── 2026-01-29_09-15-10.cfg    # Backup before ACL update
│   └── rollback-tokens.json       # Rollback metadata
----
...
[✓] Rollback complete (took 12 seconds)
...
[✗] Health check FAILED: BGP session to 10.0.1.1 down
...
[✓] Created /etc/network-orchestrator/
[✓] Created credential vault (encrypted)
[✓] Created audit log directory
[✓] Initialized Git repo for config backups
...
...
[✓] Approved by: alice@example.com at 2026-01-29 15:42:10

Creating backup of current config...
Deploying config to edge-rt2...
Validating BGP session...
[✓] BGP peer 203.0.113.10 established
...
[Diagnostics] BGP Peer 203.0.113.20
  [✓] IP reachable (ping: 12ms)
  [✓] TCP port 179 open
  [✗] BGP hold timer too low (configured: 30s, recommended: 180s)
  [✗] Route-map CUSTOMER-IN has invalid prefix-list reference
...
Backing up config...
Applying fixes...
  [✓] Set hold timer to 180s
  [✓] Created missing prefix-list PREFIX-CUSTOMER-20
  [✓] Reapplied route-map

Waiting for BGP session...
[✓] Peer 203.0.113.20 established

[✓] BGP peer repaired (flapping stopped)
----
...
Batch 1/6: core-rt1, core-rt2, edge-rt1, edge-rt2, dist-rt1
  [✓] core-rt1: deployed (12s)
  [✓] core-rt2: deployed (9s)
  [✓] edge-rt1: deployed (11s)
  [✓] edge-rt2: deployed (10s)
  [✓] dist-rt1: deployed (13s)
...
[✓] All 30 devices updated successfully
[✓] NTP sync verified on all devices
----
...
[Diagnostics] PTP on switch-dist-2
  [✓] PTP daemon running
  [✗] Network latency to grandmaster: 45ms (expected <10ms)
  [✗] PTP packets dropping (5% loss)
...
Switching PTP grandmaster to switch-core-2...
  [✓] Announced new grandmaster
  [✓] All slaves converged to switch-core-2
  [✓] Max offset now: 420ns (acceptable)

[✓] PTP grandmaster switched (incident resolved)
----
...
User reports "No internet" → Network Ambulance diagnoses "No DHCP lease" → Escalates to Network Orchestrator → Orchestrator finds "DHCP pool exhausted" → Expands pool → User gets IP address.
...
* `network-orchestrator bgp status` — read-only, no device changes
* `network-orchestrator config deploy` — prompts for approval, creates backups
...
Generating SOC2 compliance report...
  ...
  [✓] Change management audit (approvals)
  [✓] Configuration backups (retention policy)
...
* ✓ openSUSE (supported)
* ✓ Any systemd-based distro (best-effort)

Network device support:

* ✓ Cisco IOS, IOS-XE (routers, switches)
* ✓ Juniper JunOS (MX, EX, QFX series)
* ✓ Arista EOS (data center switches)
* ✓ VyOS (open-source routing)
* ✓ OpenWrt (embedded routers)
* ⚠ FRRouting (Linux routing daemon)
* ⚠ Mikrotik RouterOS (limited support)

DHCP server support:

* ✓ ISC DHCPd (Linux/Unix standard)
* ✓ Kea DHCP (modern ISC replacement)
* ✓ dnsmasq (embedded/small deployments)
* ⚠ Windows DHCP Server (limited support via PowerShell remoting)
...
network-orchestrator/
├── src/
│   ├── core/                  # Core orchestration (D language)
│   │   ├── device_manager.d   # Device inventory, SSH connections
│   │   ├── config_engine.d    # Config templating, deployment
│   │   ├── audit_logger.d     # Tamper-proof audit logging
│   │   └── rbac.d             # Role-based access control
│   ├── protocols/             # Protocol handlers (D language)
│   │   ├── bgp.d              # BGP monitoring/control
│   │   ├── snmp.d             # SNMP polling/traps
│   │   ├── dhcp.d             # DHCP management
│   │   └── ptp.d              # PTP clock synchronization
│   ├── lowlevel/              # Ultra-low-level ops (Zig)
│   │   ├── bpf_filter.zig     # eBPF packet filtering
│   │   ├── kernel_bypass.zig  # DPDK/kernel bypass for high-speed polling
│   │   └── crypto.zig         # Cryptographic signing (audit logs)
│   ├── ui/                    # User interfaces
│   │   ├── cli/               # CLI (Ada/SPARK for safety-critical)
│   │   └── gui/               # GUI (future: AffineScript + Tauri)
...
* **D** — Core orchestration, protocol handling, config management
...
* **Zig** — Ultra-low-level operations
...
* **Ada/SPARK** — Safety-critical TUI (terminal user interface)
...
* **AffineScript + Tauri** — Future GUI (cross-platform desktop app)
...
* ✓ Language selection (D, Zig, Ada/SPARK)
* 🔄 Core device manager implementation
* 🔄 BGP status monitoring (read-only)
* 🔄 SNMP polling engine
* 🔄 Basic CLI interface

=== v0.2 (Q2 2026) 🎯 **Target: Minimal Viable Product**
...
Network Orchestrator is licensed under MPL-2.0 (Palimpsest Meta-Permissive License).
...
* **Boring is good** — Reliable, predictable network operations
* **Evidence over hype** — Show real device state, not guesses
* **Safety-first** — Never break production, always have rollback
* **Respect operators** — No dark patterns, no "trust me" configs
* **Open by default** — Audit anything, inspect everything

[quote]
____
"The best network management tool is invisible—until BGP goes down at 3am, and it saves your weekend."
____
