...
* Migration: V-lang sources in `src/*.v` are deprecated and retained for reference only.
  See link:src/MIGRATION.adoc[src/MIGRATION.adoc] for the V→Zig mapping and deletion plan.

NOTE: V was the original implementation language, chosen for verification semantics.
V-lang was banned estate-wide on 2026-04-10; the Zig port preserves all safety and
non-destructive semantics. See `src/MIGRATION.adoc` for feature parity details.
