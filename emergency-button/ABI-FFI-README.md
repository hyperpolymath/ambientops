...
# EMERGENCY_BUTTON ABI/FFI Documentation
...
│  generated/abi/emergency-button.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libemergency-button.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── emergency-button.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── emergency-button.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/emergency-button.h
```
...
```c
#include "emergency-button.h"

int main() {
    void* handle = emergency-button_init();
    if (!handle) return 1;

    int result = emergency-button_process(handle, 42);
    if (result != 0) {
        const char* err = emergency-button_last_error();
...
Compile with:
```bash
gcc -o example example.c -lemergency-button -L./zig-out/lib
```

### From Idris2

```idris
import EMERGENCY_BUTTON.ABI.Foreign
...
```rust
#[link(name = "emergency-button")]
extern "C" {
    fn emergency-button_init() -> *mut std::ffi::c_void;
    fn emergency-button_free(handle: *mut std::ffi::c_void);
    fn emergency-button_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = emergency-button_init();
        assert!(!handle.is_null());

        let result = emergency-button_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libemergency-button = "libemergency-button"

function init()
    handle = ccall((:emergency-button_init, libemergency-button), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:emergency-button_process, libemergency-button), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:emergency-button_free, libemergency-button), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/emergency-button.h
   ```
