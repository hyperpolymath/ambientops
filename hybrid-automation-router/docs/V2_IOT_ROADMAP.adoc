...
*Version:* 2.0 Planning Document *Status:* Draft *Based On:*
IOT_IIOT_ARCHITECTURE.md

=== Overview

HAR v2 extends the infrastructure automation router from traditional
servers (thousands-millions) to IoT/IIoT devices (billions). This
roadmap outlines implementation phases following the architecture
specification.
...
[arabic]
. *No breaking changes* to server IaC
. IoT features are additive modules
. Existing routing tables remain compatible
. New IPv6 patterns extend routing table format
...
* link:./IOT_IIOT_ARCHITECTURE.md[IOT_IIOT_ARCHITECTURE.md] - Full
architecture
* link:./HAR_SECURITY.md[HAR_SECURITY.md] - Security model
* link:./FINAL_ARCHITECTURE.md[FINAL_ARCHITECTURE.md] - Core
architecture
* link:./CONTROL_PLANE_ARCHITECTURE.md[CONTROL_PLANE_ARCHITECTURE.md] -
Routing design
