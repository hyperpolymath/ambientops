...
This guide covers deploying HAR in a self-hosted environment without
reliance on cloud providers. Uses Podman for containerization (rootless,
daemonless) and Salt Stack for configuration management.
...
*Software Requirements:* - OS: Rocky Linux 9 / Ubuntu 22.04 LTS - Podman
4.0+ - Salt 3006+ - IPFS go-ipfs 0.20+
...
Self-hosted HAR deployment provides: - *Full control:* No cloud vendor
dependency - *Scalability:* Horizontal scaling with OTP clustering -
*Reliability:* Multi-node HA, automated failover - *Security:*
Certificate-based auth, TLS encryption - *Observability:* Prometheus +
Grafana monitoring - *Automation:* Salt Stack for config management

*Production Checklist:* - [ ] TLS certificates configured - [ ] Firewall
rules applied - [ ] Monitoring dashboards deployed - [ ] Backup
automation enabled - [ ] Load balancer health checks passing - [ ]
Cluster nodes communicating - [ ] IPFS cluster synced - [ ] Security
hardening applied - [ ] Documentation updated - [ ] Runbooks prepared
