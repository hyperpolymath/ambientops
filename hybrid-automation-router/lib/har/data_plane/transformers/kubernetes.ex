...
defmodule HAR.DataPlane.Transformers.Kubernetes do
  ...
  defp build_secret(op, namespace, _opts) do
    ...
    resource =
      if string_data != %{}, do: Map.put(resource, "stringData", string_data), else: resource
  ...
  defp build_container_spec(container) when is_map(container) do
    ...
    spec =
      add_if_present(
        spec,
        "ports",
        normalize_container_ports(container[:ports] || container["ports"])
      )
    ...
    spec =
      add_if_present(spec, "volumeMounts", container[:volume_mounts] || container["volumeMounts"])
    ...
    spec =
      add_if_present(
        spec,
        "livenessProbe",
        container[:liveness_probe] || container["livenessProbe"]
      )
    ...
    spec =
      add_if_present(
        spec,
        "readinessProbe",
        container[:readiness_probe] || container["readinessProbe"]
      )
