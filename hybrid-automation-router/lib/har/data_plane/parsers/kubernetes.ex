      ...
      if resource.kind == "Service" do
        ...
        case matching do
          [dep_id] ->
            deps ++
              [
                Dependency.new(dep_id, op.id, :requires,
                  metadata: %{reason: "k8s_service_selector"}
                )
              ]
          ...
          |> Enum.flat_map(fn name ->
            ...
            case Map.get(op_lookup, String.downcase(key)) do
              ...
              dep_id ->
                [
                  Dependency.new(dep_id, op.id, :requires,
                    metadata: %{reason: "k8s_configmap_ref"}
                  )
                ]
            end
          end)
        ...
        secret_deps =
          ...
          |> Enum.flat_map(fn name ->
            ...
            case Map.get(op_lookup, String.downcase(key)) do
              ...
              dep_id ->
                [Dependency.new(dep_id, op.id, :requires, metadata: %{reason: "k8s_secret_ref"})]
            end
          end)
    ...
    deps =
      if resource.namespace != "default" do
        ...
        case Map.get(op_lookup, String.downcase(ns_key)) do
          ...
          dep_id ->
            deps ++
              [Dependency.new(dep_id, op.id, :requires, metadata: %{reason: "k8s_namespace"})]
        end
      else
        deps
      end
