...
defmodule HAR.DataPlane.Parsers.Chef do
  ...
  defp normalize_resource_type("apt_package", action),
    do: normalize_resource_type("package", action)

  defp normalize_resource_type("yum_package", action),
    do: normalize_resource_type("package", action)

  defp normalize_resource_type("dnf_package", action),
    do: normalize_resource_type("package", action)
        ...
        case notification do
          {:notifies, _action, resource_ref, _timing} ->
            ...
            case Map.get(op_lookup, key) do
              ...
              target_id ->
                [
                  Dependency.new(op.id, target_id, :notifies,
                    metadata: %{reason: "chef_notifies"}
                  )
                ]
            end
          ...
          {:subscribes, _action, resource_ref, _timing} ->
            ...
            case Map.get(op_lookup, key) do
              ...
              source_id ->
                [
                  Dependency.new(source_id, op.id, :watches,
                    metadata: %{reason: "chef_subscribes"}
                  )
                ]
            end
        end
