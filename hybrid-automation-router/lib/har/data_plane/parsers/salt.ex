defmodule HAR.DataPlane.Parsers.Salt do
  ...
  defp extract_state_dependencies(operation, state_data, all_operations)
       when is_map(state_data) do
    state_data
    |> Enum.flat_map(fn
  ...
  defp extract_deps(operation, requirements, dep_type, all_operations)
    ...
    |> Enum.map(fn req ->
      case find_operation_by_requirement(req, all_operations) do
        ...
        required_op ->
          Dependency.new(required_op.id, operation.id, dep_type, metadata: %{salt_requisite: req})
      end
    end)
  end
