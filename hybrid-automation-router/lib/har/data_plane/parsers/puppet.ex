...
defmodule HAR.DataPlane.Parsers.Puppet do
  ...
  def validate(content) when is_binary(content) do
    ...
    cond do
      open_braces != close_braces ->
        {:error,
         {:puppet_parse_error, "Unbalanced braces: #{open_braces} open, #{close_braces} close"}}
  ...
  defp extract_standard_resources(content) do
    ...
    # Negative lookbehind prevents matching @type (virtual) or @@type (exported),
    # and prevents partial matches within longer identifiers
    resource_regex =
      ~r/(?<![a-z@])([a-z][a-z0-9_]*)\s*\{\s*(?:'([^']+)'|"([^"]+)"|(\$?[a-z][a-z0-9_]*))\s*:\s*([^}]+)\}/is
  ...
  defp extract_virtual_resources(content) do
    # Match: @type { 'title': ... }
    # Negative lookbehind prevents matching @@type (exported resources)
    virtual_regex =
      ~r/(?<!@)@([a-z][a-z0-9_]*)\s*\{\s*(?:'([^']+)'|"([^"]+)"|(\$?[a-z][a-z0-9_]*))\s*:\s*([^}]+)\}/is
  ...
  defp parse_attributes(body) do
    ...
    |> Enum.map(fn [_full, key, value] ->
      cleaned = value |> String.trim() |> String.trim_trailing(",") |> String.trim()
      {String.trim(key), parse_value(cleaned)}
    end)
    |> Map.new()
  end
  ...
  defp extract_balanced_block(["{" | rest], depth, acc),
    do: extract_balanced_block(rest, depth + 1, ["{" | acc])

  defp extract_balanced_block(["}" | _rest], 1, acc), do: Enum.join(Enum.reverse(acc))

  defp extract_balanced_block(["}" | rest], depth, acc) when depth > 1,
    do: extract_balanced_block(rest, depth - 1, ["}" | acc])

  defp extract_balanced_block([char | rest], depth, acc),
    do: extract_balanced_block(rest, depth, [char | acc])
  ...
  defp extract_explicit_relationships(operations, op_lookup) do
    ...
    |> Enum.flat_map(fn op ->
      ...
      requires =
        extract_relationship_refs(attrs, "require", op_lookup)
        |> Enum.map(fn dep_id ->
      ...
      befores =
        extract_relationship_refs(attrs, "before", op_lookup)
        |> Enum.map(fn dep_id ->
      ...
      notifies =
        extract_relationship_refs(attrs, "notify", op_lookup)
        |> Enum.map(fn dep_id ->
      ...
      subscribes =
        extract_relationship_refs(attrs, "subscribe", op_lookup)
        |> Enum.map(fn dep_id ->
  ...
  defp extract_chaining_arrows(content, op_lookup) do
    ...
    |> Enum.flat_map(fn [_full, type1, title1, arrow, type2, title2] ->
      ...
      if from_id && to_id do
        ...
        [
          Dependency.new(from_id, to_id, dep_type,
            metadata: %{reason: "puppet_chaining_#{arrow}"}
          )
        ]
      else
        []
