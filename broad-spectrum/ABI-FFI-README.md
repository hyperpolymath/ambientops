...
# BROAD_SPECTRUM ABI/FFI Documentation
...
│  generated/abi/broad-spectrum.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libbroad-spectrum.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── broad-spectrum.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── broad-spectrum.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/broad-spectrum.h
```
...
```c
#include "broad-spectrum.h"

int main() {
    void* handle = broad-spectrum_init();
    if (!handle) return 1;

    int result = broad-spectrum_process(handle, 42);
    if (result != 0) {
        const char* err = broad-spectrum_last_error();
...
Compile with:
```bash
gcc -o example example.c -lbroad-spectrum -L./zig-out/lib
```

### From Idris2

```idris
import BROAD_SPECTRUM.ABI.Foreign
...
```rust
#[link(name = "broad-spectrum")]
extern "C" {
    fn broad-spectrum_init() -> *mut std::ffi::c_void;
    fn broad-spectrum_free(handle: *mut std::ffi::c_void);
    fn broad-spectrum_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = broad-spectrum_init();
        assert!(!handle.is_null());

        let result = broad-spectrum_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libbroad-spectrum = "libbroad-spectrum"

function init()
    handle = ccall((:broad-spectrum_init, libbroad-spectrum), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:broad-spectrum_process, libbroad-spectrum), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:broad-spectrum_free, libbroad-spectrum), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/broad-spectrum.h
   ```
