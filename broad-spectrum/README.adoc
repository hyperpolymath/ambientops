...
A comprehensive CLI website auditor built with AffineScript and Deno. Performs automated security, accessibility, performance, and SEO audits on websites.
...
- Type-safe AffineScript core with zero runtime errors
...
2. *Install Node.js & npm* (for building AffineScript):
```bash
...
3. *Install AffineScript* (globally):
```bash
...
- *AffineScript*: Type-safe functional language compiling to JavaScript (100% of application code)
...
```bash
# Install dependencies (AffineScript compiler)
npm install
...
1. *Language Policy*: Use AffineScript only (no TypeScript)
...
- Install AffineScript globally: `npm install -g affinescript`
...
Built with:
- [AffineScript](https://affinescript-lang.org/) - Type-safe functional language
