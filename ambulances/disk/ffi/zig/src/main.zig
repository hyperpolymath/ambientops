...
const VERSION = "0.1.0";
const BUILD_INFO = "AMBULANCES built with Zig " ++ @import("builtin").zig_version_string;
...
export fn ambulances_init() ?*Handle {
...
export fn ambulances_free(handle: ?*Handle) void {
...
export fn ambulances_process(handle: ?*Handle, input: u32) Result {
...
export fn ambulances_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn ambulances_free_string(str: ?[*:0]const u8) void {
...
export fn ambulances_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
export fn ambulances_last_error() ?[*:0]const u8 {
...
export fn ambulances_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn ambulances_build_info() [*:0]const u8 {
...
/// Register a callback
export fn ambulances_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn ambulances_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(ambulances_is_initialized(handle) == 1);
}

test "error handling" {
    const result = ambulances_process(null, 0);
