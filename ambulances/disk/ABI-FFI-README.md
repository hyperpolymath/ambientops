...
# AMBULANCES ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libambulances.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── ambulances.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── ambulances.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/ambulances.h
```
...
int main() {
    void* handle = ambulances_init();
    if (!handle) return 1;

    int result = ambulances_process(handle, 42);
    if (result != 0) {
        const char* err = ambulances_last_error();
...
Compile with:
```bash
gcc -o example example.c -lambulances -L./zig-out/lib
```

### From Idris2

```idris
import AMBULANCES.ABI.Foreign
...
```rust
#[link(name = "ambulances")]
extern "C" {
    fn ambulances_init() -> *mut std::ffi::c_void;
    fn ambulances_free(handle: *mut std::ffi::c_void);
    fn ambulances_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = ambulances_init();
        assert!(!handle.is_null());

        let result = ambulances_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libambulances = "libambulances"

function init()
    handle = ccall((:ambulances_init, libambulances), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:ambulances_process, libambulances), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:ambulances_free, libambulances), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/ambulances.h
   ```
